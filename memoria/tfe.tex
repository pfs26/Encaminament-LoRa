...
% TODO: AQUI NO ES SERIA NECESSARI PERQUE LA RUTA SEMPRE ÉS LA MATEIXA!
...
Per permetre aquest comportament, es guarda informació addicional d'una transmissió fiable, en una estructura de metadades:
\begin{itemize}
    ...
    \item \emph{RX}. L'adreça del receptor del segment. \textcolor{red}{TODO: AIXÒ NO ÉS NECESSARI, VEURE ISSUE A GITHUB}
...
\begin{table}
    \begin{tabular}{ccc}
        \toprule
        \textbf{Port} & \textbf{Transmissió} & \textbf{Recepció} \\
        \midrule
        \fitx{00} & \fitx{TxDone1()} & \fitx{Rx1()} \\
        \fitx{17} & \fitx{TxDone2()} & \fitx{Rx2()} \\
        % \multicolumn{3}{c}{\dots} \\
        \fitx{63} & \fitx{TxDoneN()} & \fitx{RxN()} \\
        \bottomrule
    \end{tabular}
    \centering
...
El temps d'espera de recepció del reconeixement no és fix, sinó que depèn del nombre de retransmissions que s'han realitzat anteriorment, seguint una funció exponencial. Aquest temps ve determinat per l'expressió $k\cdot 2^r$, on \emph{k} és un valor constant i configurable, i \emph{r} el nombre de reintents realitzats.
% EXPLICAR COM S'HAURIA DE DEFINIR K (tenitn en compte dispositius a la xarxa, llargada màxima de la ruta, salts màxims, transmissió més lenta per WAN, etc.)

En aquest moment, es verifica si s'ha rebut el reconeixement. Si no s'ha rebut, es torna a transmetre el segment, incrementant el nombre de reintents. Si s'ha arribat al nombre màxim de reintents, es descarta el segment i es notifica a la capa superior.
