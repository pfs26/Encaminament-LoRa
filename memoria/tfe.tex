...
\begin{itemize}
    ...
    $$t_t =  (N \cdot (R+1) \cdot (t_{{tx}} + K_{\text{ack}} \cdot t_{\text{ack}})) \cdot (1 + f)$$
    \begin{itemize}
        \item $N$: nombre de nodes anteriors al dispositiu pel qual es calcula el temps de tolerància.
        ...
        \item $t_{tx}$: temps de transmissió de les dades, considerant els paràmetres LoRa i la mida màxima de les dades (\SI{255}{\byte}).
        \item $K_{\text{ack}}$: factor de temps d'espera per la recepció del missatge de reconeixement de la capa d'accés al medi. Veure \autoref{subsubsec:integritat}.
        \item $t_{\text{ack}}$: temps de transmissió del missatge de reconeixement de la capa d'accés al medi, considerant la mida d'aquest (\SI{7}{\byte}, la mida dels \est{headers} d'una trama d'accés al medi).
    ...
    \begin{itemize}
        ...
        \item En rebre'ls tots, afegir les seves dades ---les de la lectura de dispositiu--- a l'últim segment (fragmentant-lo si és necessari), i transmetre com s'ha vist anteriorment.
    \end{itemize}

    % // TODO: Afegir diagrama de fragmentacions?
    % // TODO: A PARTIR D'AQUI QUEDA PENDENT REVISAR

    \item \emph{Pèrdua de dades.} Tot i existir mecanismes que intenten assegurar la integritat dels missatges, és possible que un dispositiu no rebi correctament les dades. Això pot ser degut a interferències, o bé a que el dispositiu que ha d'enviar les dades s'ha espatllat
