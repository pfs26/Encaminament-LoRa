...
La funció d'aquesta capa és la de determinar quin és el següent dispositiu a qui s'ha d'enviar un missatge per fer-lo arribar al destí final. Com s'ha comentat, s'utilitza encaminament estàtic i, per tant, les taules que defineixen les rutes s'han de definir prèviament. És també responsable de gestionar i accedir a la taula. 

Una altra tasca important que realitza és la de commutar entre utilitzar el protocol definit, a través de la capa d'accés al medi, o utilitzar LoRaWAN, a través de la capa d'abstracció de LoRaWAN prèviament comentada. Així, aquesta capa es comunica amb la capa d'accés al medi, rebent notificacions de fi de transmissió i de recepció de missatges a través del protocol personalitzat, i amb la capa d'abstracció de LoRaWAN, rebent notificacions de recepció de missatges a través de LoRaWAN.

\subsubsection{Estructura de les trames}
És necessari que aquesta capa pugui saber quin és el destí final del missatge, per tal de determinar quin és el següent dispositiu a qui s'ha d'enviar. Per tant, ha de contenir un camp amb l'adreça del dispositiu de \emph{destí}. També s'inclou l'adreça del dispositiu \emph{emissor}, permetent saber d'on s'ha originat el missatge. Aquests camps tenen la mateixa longitud que els camps d'adreça de la capa d'accés al medi, d'\SI{1}{\byte}, per tal de poder utilitzar la mateixa adreça en ambdues capes, simplificant la implementació.

Per tal d'evitar transmissions infinites, per exemple si una ruta resulta en un bucle, s'inclou un camp de \acro{ttl} (\est{Time To Live}), que indica el nombre màxim de salts que pot fer un missatge abans de ser descartat. Per exemple, si l'emissor genera un missatge amb un \acro{ttl} d'1, el missatge només es transmetrà al següent dispositiu de la ruta. S'ha definit d'una longitud màxima d'\SI{1}{\byte}, permetent un màxim de 255 salts. Amb aquest límit, podria recórrer tots els dispositius de la xarxa (limitats a 253), així que no és necessaria una longitud major.

Per acabar, s'ha inclòs un camp de \emph{longitud}, que indica la longitud de les dades del missatge. Com en el protocol d'accés al medi, permet saber quants bytes s'han de llegir per obtenir les dades. Es reserva també un byte, ja que la longitud màxima de les dades del protocol d'accés al medi és de \SI{248}{\byte}. Així, les \emph{dades} podran tenir una longitud màxima de \SI{243}{\byte}, sent els \SI{4}{\byte} restants per a la resta de camps.

Seguint el model \acro{tcp/ip}, el missatges d'aquesta capa s'anomenen \emph{paquets}. La seva estructura, amb els camps prèviament mencionats, es pot observar a la \autoref{fig:paquet_encaminament}.
