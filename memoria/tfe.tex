...
Les proves s'han realitzat utilitzant dos microcontroladors \acro{esp32}, cadascun equipat amb un transductor LoRa \acro{sx1262}. Per verificar el correcte funcionament a través de LoRaWAN, es disposava de connectivitat a un \est{gateway} Mikrotik wAP-LR8. Es poden observar els dispositius utilitzats a la \autoref{fig:dispositius_proves}.
...
Per descobrir l'origen d'aquest error, es va dur a terme una bateria de proves (\fitx{tests/provesOrigenErrorClock.cpp}). Les proves consistien en simular el funcionament de la capa d'aplicació per a cada dispositiu, però sense utilitzar-la directament. Les proves realitzades van ser:
\begin{itemize}
...
En mode actiu, el dispositiu té un consum d'aproximadament \SI{80}{\milli\ampere}. En mode de baix consum, en canvi, aquest es redueix fins aproximadament \SI{13}{\milli\ampere}. És important tenir en compte que la placa de desenvolupament conté un \acro{led} vermell, el qual sempre es troba actiu, i presenta un consum afegit. En qualsevol dels casos, es pot observar com efectivament el consum s'ha vist reduït.
