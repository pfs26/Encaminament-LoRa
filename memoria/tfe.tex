...
Sobre aquest protocol s'ha desenvolupat una aplicació optimitzada per a xarxes lineals ---una topologia especialment útil en vies de tren o túnels--- centrada en minimitzar el consum energètic dels dispositius. Aquesta solució es basa en una estratègia de sincronització entre nodes, que permet coordinar-ne les transmissions i reduir el temps actiu dels dispositius, sense renunciar a la fiabilitat ni als mecanismes d'encaminament que el protocol ofereix. Les proves realitzades han demostrat que la proposta és viable, escalable i eficient en entorns on el consum energètic i la cobertura són crítics.
\end{resum}


\begin{abstract}
This work presents the design and impementation of a communication protocol based on LoRa and static routing, inteded for monitoring device networks in extensive or isolated environments, where human presence is limited. The goal is to reduce the dependency on LoRaWAN gateways, taking advantage of the fact that devices are usually static and allow the use of predefined communication routes.
...
Tecnologies com LoRaWAN han guanyat protagonisme en aquests escenaris, gràcies a la seva capacitat de transmetre dades a llargues distàncies amb un consum energètic reduït. No obstant això, solen requerir de \est{gateways}, dispositius que actuen com a punt d'accés entre els nodes ---els dispositius de monitoratge--- i internet. Aquests \est{gateways} són més complexos i cars que els dispositius finals, fent que en grans insta\l.lacions el seu ús pugui resultar en costos elevats i tasques de manteniment més complexes.

Aquest treball té com a objectiu dissenyar una solució per a xarxes de dispositius de monitoratge que permeti reduir la dependència dels \est{gateways}, partint de la premissa que aquestes xarxes són, en general, estàtiques, permetent utilitzar rutes de comunicació prèviament definides. 
