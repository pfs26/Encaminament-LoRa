...
\begin{resum}
Aquest treball presenta el disseny i implementació d'un protocol de comunicació modular basat en LoRa i amb encaminament estàtic, pensat per a xarxes de dispositius de monitoratge en entorns extensos o aïllats, on la presència humana és limitada. L'objectiu és reduir la dependència dels \est{gateways} LoRaWAN, aprofitant que els dispositius són habitualment estàtics i permeten l'ús de rutes de comunicació predefinides.

Sobre aquest protocol s'ha desenvolupat una aplicació optimitzada per a xarxes lineals ---una topologia especialment útil en vies de tren o túnels--- centrada a minimitzar el consum energètic dels dispositius. Aquesta solució es basa en una estratègia de sincronització entre nodes, que permet coordinar-ne les transmissions i reduir el temps actiu dels dispositius, sense renunciar a la fiabilitat ni als mecanismes d'encaminament que el protocol ofereix. Les proves realitzades han demostrat que la proposta és viable, escalable i eficient en entorns on el consum energètic i la cobertura són crítics.
\end{resum}


\begin{abstract}
This work presents the design and implementation of a modular and layered communication protocol based on LoRa and static routing, intended for monitoring device networks in extensive or isolated environments, where human presence is limited. The goal is to reduce the dependency on LoRaWAN \est{gateways}, taking advantage of the fact that devices are usually static and allow the use of predefined communication routes.
...
Tecnologies com LoRaWAN han guanyat protagonisme en aquests escenaris, gràcies a la seva capacitat de transmetre dades a llargues distàncies amb un consum energètic reduït. No obstant això, solen requerir de \est{gateways}, dispositius que actuen com a punt d'accés entre els nodes ---els dispositius de monitoratge--- i internet. Aquests \est{gateways} són més complexos i cars que els dispositius finals, fent que en grans insta\l.lacions el seu ús pugui provocar costos elevats i tasques de manteniment més complexes.

Aquest treball té com a objectiu dissenyar una solució per a xarxes de dispositius de monitoratge que permeti reduir la dependència dels \est{gateways}, partint de la premissa que aquestes xarxes són, en general, estàtiques, cosa que permet utilitzar rutes de comunicació prèviament definides. 

El resultat del treball ha estat el disseny i desenvolupament d'un protocol de comunicació fiable i modular, basat en tecnologia LoRa, amb encaminament estàtic i compatible amb LoRaWAN. A més, s'ha implementat una aplicació de baix consum energètic, construïda sobre aquest protocol, dissenyada específicament per a topologies lineals i basada en una estratègia de sincronització entre dispositius. 

Les validacions s'han dut a terme en un entorn controlat, simulant escenaris reals, i han demostrat que el protocol és capaç de transmetre dades de manera fiable i eficient. A més, l'aplicació de baix consum energètic ha demostrat una reducció significativa en el consum dels dispositius finals.
...
En un primer moment, es va plantejar un únic objectiu, que era dissenyar un protocol de comunicació modular i fiable, basat en LoRa, que permetés reduir la dependència dels \est{gateways} LoRaWAN. No obstant això, a mesura que es va avançar en el projecte, es va veure la importància de minimitzar el consum dels dispositius finals. D'aquesta manera, els objectius van acabar sent:
\begin{itemize}
    \item Dissenyar i implementar un protocol amb encaminament estàtic per a xarxes de dispositius sense mobilitat, pensat per reduir la necessitat de \est{gateways} i optimitzar la recollida de dades en entorns extensos.
...
% EStructura: 3 parts, una teòria, una pel protocol, i una per l'aplicació
...
\begin{itemize}
    \item \emph{Amplada de banda}. És l'amplada de banda (\acro{bw}) del canal de transmissió, sovint de \SI{125}{\kHz}, però també pot ser de \SI{250}{\kHz} o \SI{500}{\kHz}. Una amplada de banda més gran permet una major velocitat de transmissió, però redueix la sensibilitat de recepció i, per tant, el rang de comunicació.
    \item \emph{Data rate}. És la velocitat de transmissió de dades (\acro{dr}), que depèn de l'amplada de banda i del factor d'espargiment. Un  \emph{data rate} més alt permet una major velocitat de transmissió, però redueix la sensibilitat de recepció i, per tant, el rang de comunicació.
...
\begin{itemize}
    ...
    \item \emph{Classe C}. Obren també dues finestres de recepció després d'un \est{uplink}, amb la diferència que la segona finestra es manté oberta fins a la següent transmissió. Així, es pot considerar que aquests dispositius sempre estan rebent, amb l'excepció de quan transmeten. La latència de recepció de \est{downlinks} és la mínima d'entre les tres classes, però el consum energètic és màxim.
\end{itemize}

Una altra característica important de LoRaWAN és la seva capacitat d'autoajustament, conegut com a \acro{adr} (\est{Adaptive Data Rate}). Els dispositius finals poden ajustar automàticament els paràmetres de comunicació, com el \acro{sf} o la potència de transmissió, per adaptar-se a les condicions del canal de comunicació. Quan s'utilitza, el servidor de la xarxa pot indicar als dispositius finals quins paràmetres utilitzar, reduint el consum energètic i possibles interferències amb altres dispositius. Per exemple, un dispositiu molt proper a un \est{gateway} hauria d'utilitzar \acro{sf} més baix, mentre que dispositius llunyans n'haurien d'utilitzar de més elevats.

\subsection{The Things Network}
Es tracta d'un projecte d'\acro{iot} que proporciona un servidor de xarxa públic i gratuït. El seu objectiu és crear una xarxa global de dispositius LoRaWAN, que permet la comunicació entre ells sense necessitat d'infraestructura pròpia. Els usuaris poden connectar els seus \est{gateways} a la xarxa i compartir la seva cobertura amb altres usuaris. Es pot consultar tota la seva documentació, que inclou referències de LoRaWAN, a \cite{the_things_network_lorawan_nodate}.
...
En utilitzar la modulació LoRa, que com s'ha dit anteriorment utilitza freqüències \acro{ism}, intenta maximitzar l'eficiència d'ús del canal. Això redueix la generació d'interferències amb altres dispositius que facin servir el mateix canal, el nombre de retransmissions, i un menor consum energètic.

Ofereix compatibilitat amb LoRaWAN, permetent que dispositius amb connexió a \est{gateways} puguin alternar entre l'ús del protocol dissenyat o LoRaWAN. Això ofereix la possibilitat a dispositius que no tenen connexió directa a \est{gateways} de comunicar-s'hi a través de dispositius que sí que en tenen.
...
Finalment, per facilitar la configuració de tot el protocol, s'ha definit un únic fitxer de configuració (\fitx{config.h}), on es poden configurar tots els paràmetres del protocol, evitant la seva dispersió en diverses parts del codi.
...
Pel que fa a l'entorn de desenvolupament, s'ha utilitzat PlatformIO, integrat dins de Visual Studio Code. Aquest facilita la gestió de llibreries, és compatible amb múltiples plataformes i microcontroladors, i integra eines de depuració i monitoratge del port sèrie, facilitant el desenvolupament. 

\section{Disseny per capes}
Com en la majoria dels protocols moderns, s'ha dissenyat seguint un model per capes. L'estructura es basa en el model \acro{tcp/ip}, amb la diferència de la capa física, que el model \acro{tcp/ip} no contempla de forma explícita.
...
La seva implementació és senzilla, exposant únicament dues interfícies que permeten configurar el transductor en mode LoRa o LoRaWAN, i dues més que permeten inicialitzar o desinicialitzar el transductor. La implementació es pot consultar a \fitx{lora.cpp}.
...
Dos camps essencials són el \emph{receptor} i l'\emph{emissor}, que indiquen respectivament les adreces del dispositiu que rep i envia el missatge. S'ha decidit utilitzar adreces d'\SI{1}{\byte}, oferint un total de 256 adreces diferents. Malgrat això, únicament 253 són utilitzables, reservant l'adreça \fitx{0x01} per representar el \est{gateway}, la \fitx{0xFF} per un possible ús de difusió (\emph{broadcast}), i la \fitx{0x00}, per indicar l'absència d'adreça. Aquesta quantitat de dispositius es considera suficient per a la majoria d'aplicacions de monitoratge, evitant l'ús innecessari de dades i, per tant, minimitzant el temps de transmissió i el consum energètic.

Per a identificar cada trama, s'ha inclòs un camp d'\emph{identificador} (\emph{id}), que conté un nombre aleatori generat per l'emissor. Aquest camp permet que el receptor pugui identificar trames duplicades i no processar-les de nou. S'ha considerat utilitzar una longitud de \SI{2}{\byte}, permetent \num{65536} identificadors diferents. Tot i això, s'ha reservat l'identificador \fitx{0x0000}; la utilitat d'aquest s'exposarà al \autoref{subsubsec:mac_lorawan}. En el pitjor dels casos, si \num{252} dispositius transmetessin a un mateix receptor, la probabilitat d'una co\l.lisió és, aproximadament, d'un 38\%. Es tracta d'un problema similar al del «problema dels aniversaris» \cite{noauthor_birthday_2025}.

Tot i semblar una probabilitat molt elevada, no es considera un fet greu: es tracta del pitjor dels casos, on \num{252} dispositius transmeten al mateix destí, i on el receptor recorda tots els identificadors rebuts. En aplicacions reals, la quantitat d'identificadors guardats és limitada (per exemple als últims 10), reduint la probabilitat de co\l.lisió. Si ho repetim en un cas més realista, on hi ha un receptor comú i 5 transmissors, obtenim una probabilitat de col\l.lisió del 0.015\%, i del 0.068\% si considerem 10 transmissors. Així, s'ha considerat escaient una longitud de \SI{2}{\byte}, buscant minimitzar el consum energètic i el temps de transmissió, sense afectar greument el funcionament.
...
Una característica dels missatges de reconeixement és que l'identificador no és aleatori, sinó que és el mateix que el missatge que reconeix, l'«original». Això permet que el transmissor pugui identificar correctament l'\acro{ack}, i sàpiga a quines dades fa referència. Si no fos així, el transmissor podria rebre un reconeixement d'una transmissió que havia considerat fallida, després d'haver-ne realitzat una altra; com que no sabria que fa referència a la primera, pensaria que la segona s'ha realitzat correctament. A més, el camp d'\acro{ack} dels \est{flags} està actiu, i el camp de dades no conté informació.
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{imatges/mac_FSM.drawio.pdf}
...
\begin{itemize}
    \item \emph{Idle}. Estat inicial. Es troba esperant una transmissió. En produir-se, comprova l'estat del canal, i si està lliure inicia la transmissió i el temporitzador d'espera de reconeixement. Si el canal està ocupat, s'inicia el temporitzador d'accés al canal (\acro{beb}).
    \item \emph{Wait ack}. Es troba esperant el reconeixement. En produir-se un \emph{timeout}, fi del temporitzador, envia de nou si encara hi ha intents disponibles. Si el canal llavors es troba ocupat, inicia el temporitzador d'accés al canal; en cas contrari, transmet i inicia de nou el temporitzador d'espera de reconeixement. Si s'han esgotat els reintents, o es rep l'\acro{ack}, notifica d'error de transmissió o de fi de transmissió, respectivament. El \emph{callback} de recepció de la capa inferior és l'encarregat de generar l'esdeveniment de recepció d'\acro{ack}.
    ...
    \item \emph{Queue}. Verifica l'estat de la cua de transmissió. Si es troba buida, torna a l'estat \emph{idle}. En cas contrari, si el canal es troba lliure inicia la transmissió, i si no aplica \acro{beb}.
\end{itemize}

Com la resta del protocol, s'ha implementat de forma no bloquejant ---amb l'excepció de la transmissió a través de la capa inferior, que sí que és bloquejant---, utilitzant el gestor de tasques. S'han utilitzat tasques per programar els temps d'espera de recepció de reconeixement, i els de l'accés al canal (\acro{beb}), i també per programar l'execució d'esdeveniments de la màquina d'estats principal. Això permet que l'execució de la \acro{fsm} no afecti altres tasques si el temps d'execució és molt llarg.
...
S'utilitzen dispositius LoRaWAN de classe A, i la definició 1.1 del protocol LoRaWAN, la qual afegeix millores de seguretat respecte de les versions \acro{1.0.x} anteriors. Es pot consultar l'especificació completa a \cite{noauthor_lorawan_2023}.
...
Una situació més excepcional és la representada a la \autoref{fig:mac_tx_considerat_fallida}, on el dispositiu 1 realitza una transmissió, sense rebre el missatge de reconeixement en cap reintent. Malgrat això, el dispositiu 2 sí que havia rebut la trama, generant l'\acro{ack} corresponent, el qual no arriba al dispositiu 1. S'observa com en la segona recepció de la trama, el dispositiu 2 no notifica a la capa superior, perquè ja s'havia processat prèviament. En aquesta situació, el dispositiu 1 considera que la transmissió no ha estat satisfactòria, malgrat que el dispositiu 2 sí que havia rebut la trama.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Comportament complex a la capa d'accés al medi.}
...
Seguint el model \acro{tcp/ip}, els missatges d'aquesta capa s'anomenen \emph{paquets}. La seva estructura, amb els camps prèviament esmentats, es pot observar a la \autoref{fig:paquet_encaminament}.
...
A la \autoref{fig:routing_encaminament_basic_gateway} es mostra un exemple bàsic. Primerament, el dispositiu \fitx{0x02} vol enviar un paquet al \fitx{0x04}: obté la ruta, i el transmet a \fitx{0x03}. Aquest rep el paquet, verifica el \acro{ttl}, i obté la següent ruta, que és \fitx{0x04}. Quan aquest rep el paquet, comprova que és el destí final, i ho notifica a la capa superior. 

Un segon exemple de transmissió es realitza quan \fitx{0x02} vol enviar un paquet al \est{gateway}. El dispositiu 2 consulta la taula d'encaminament i veu que el següent salt és el dispositiu 3. Aquest rep el paquet, consulta la seva taula, i ho transmet al dispositiu \fitx{0x04}, qui ho encamina cap al \est{gateway} a través de LoRaWAN. En obrir les finestres de recepció, rep un paquet amb el dispositiu \fitx{0x03} com a destí, i utilitza el mateix procediment vist anteriorment per fer-li arribar el paquet.
...
Una altra situació més complexa es pot produir quan la xarxa es troba separada en dues, amb un \est{gateway} LoRaWAN al mig. En aquest cas, el comportament és similar a l'observat anteriorment, on el \est{gateway} fa les transmissions dels paquets únicament després de rebre'n un altre, ja que es tracten de dispositius de classe A. Així, si un dispositiu vol enviar un paquet a un dispositiu de la xarxa contrària, la correcta transmissió d'aquest paquet dependrà de la transmissió d'un altre paquet originat a l'altra xarxa, per tal que pugui obrir la finestra de recepció necessària.

A més, es requereix també un servidor d'aplicació, encarregat de processar els paquets que el \est{gateway} rep i generar els \est{downlinks} corresponents. Malgrat aquests requisits, aquest exemple permet i\l.lustrar les capacitats d'encaminament de la capa, que no tenen per què estar limitades a una única xarxa. El seu diagrama es pot trobar representat a la \autoref{fig:routing_encaminament_complex_gateway}.
...
\end{figure}
\subsubsection{Transmissió}
Com en la capa d'encaminament, la transmissió en la capa de transport és simple: utilitza la interfície exposada per la capa d'encaminament. En finalitzar la transmissió, la capa d'encaminament notificarà a través d'un \est{callback} aquesta finalització. 
...
El temps d'espera de recepció del reconeixement no és fix, sinó que depèn del nombre de retransmissions que s'han realitzat anteriorment, seguint una funció exponencial. Gràcies a això, el temps d'espera s'incrementa amb el nombre de reintents, ajustant-se a les condicions de la xarxa.
Aquest temps és determinat per l'expressió $k\cdot 2^r$, on \emph{k} és un valor constant i configurable, i \emph{r} el nombre de reintents realitzats. El valor de \emph{k} cal configurar-lo adequadament per evitar generar \est{timeouts} prematurs, i per evitar un excés de temps d'espera:
\begin{itemize}
...
\end{itemize}
Aquest criteri estableix el temps mínim d'espera, considerant que per a cada dispositiu de la ruta es requereix únicament una transmissió, i que el retard de processament és nul. Si s'estableix un temps mínim d'espera menor a aquest, és segur que es produiran \est{timeouts} prematurs.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Exemple funcionament capa de transport.}
...
Les proves s'han realitzat de forma manual, observant l'estat del sistema a través de les traces, i verificant el comportament del protocol a través dels diagrames de funcionament previst. Part de les proves han requerit intervenció manual, com ara la desconnexió de dispositius (per simular-ne una fallada), o la programació d'enviaments de \est{downlinks} a través de \acro{ttn}. A continuació es pot observar un exemple del registre de funcionament (\fitx{exemples/Transport/FiableWAN_NoFiableRAW.cpp}) generat pel dispositiu sense capacitats LoRaWAN:

{
    \tiny
\begin{lstlisting}[frame=single]
[I]   38 | [TRANSPORT] Initializing...
[I]  211 | [TRANSPORT] Preparing to send
[I]  222 | [TRANSPORT] SEGMENT: ID=10645 PORT=63 ACKreq=1 ACKResp=0 D-LEN=5 DATA=Hello
[I]  232 | [ROUTING] Sending packet:
[I]  232 | [ROUTING] PACKET: SRC=03 DST=01 TTL=5 D-LEN=9 DATA=)Hello
[I]  242 | [RTABLE] Queried route for 0x01, through 0x02
[I]  253 | [MAC] Preparing to send
[I]  263 | [MAC] FRAME: TX=03 RX=02 ID=58661 D-LEN=13 DATA=)Hello CRC=46 ACK=0 RETRY=0
[I]  274 | [MAC] FSM transmission scheduled
[I]  284 | [ROUTING] Added packet to send queue (ID: 58661)
[I]  295 | [TRANSPORT] Scheduled to send segment (Segment ID: 10645, Frame ID: 58661)
[I]  313 | [LR] Set power to -9 dBm
[I]  314 | [LR] Preparing to send
[I]  333 | [LR] Packet: 030225E5F80D030105099529FD0548656C6C6F2E
[I]  402 | [LR] Started Receiving
[I]  403 | [MAC] Frame sent successfully, waiting for ACK
[I]  419 | [LR] Started Receiving
[I]  419 | [MAC] Timeout d'ACK: 108ms (36096us airtime)
[I]  510 | [LR] Data received. SNR: 12, RSSI: -59
[I]  510 | [MAC] Frame rcv
[I]  527 | [LR] Started Receiving
[I]  527 | [LR] Received valid PDU from LORA
[I]  537 | [MAC] FRAME: TX=02 RX=03 ID=58661 D-LEN=0 DATA= CRC=211 ACK=1 RETRY=0
[I]  548 | [MAC] ACK Received from 0x02
[I]  556 | [MAC] ACK received
[I]  556 | [MAC] Sent. Notify higher layer?
[I]  572 | [LR] Started Receiving
[I]  572 | [TRANSPORT] Frame 58661 sent. Waiting 10000 ms for ACK. Running at 10567ms
[I] 6323 | [LR] Data received. SNR: 11, RSSI: -59
[I] 6323 | [MAC] Frame rcv
[I] 6340 | [LR] Started Receiving
[I] 6340 | [LR] Received valid PDU from LORA
[I] 6351 | [MAC] FRAME: TX=02 RX=03 ID=35527 D-LEN=8 DATA=) CRC=28 ACK=0 RETRY=0
[I] 6361 | [MAC] Frame for higher layer
[I] 6362 | [LR] Set power to -9 dBm
[I] 6372 | [LR] Preparing to send
[I] 6391 | [LR] Packet: 0302C78AF90052
[I] 6440 | [LR] Started Receiving
[I] 6451 | [ROUTING] Received packet from 0x02
[I] 6451 | [ROUTING] PACKET: SRC=02 DST=03 TTL=5 D-LEN=4 DATA=)
[I] 6461 | [TRANSPORT] Received segment
[I] 6472 | [TRANSPORT] SEGMENT: ID=10645 PORT=0 ACKreq=0 ACKResp=1 D-LEN=0 DATA=
[I] 6482 | [TRANSPORT] ACK not requested for segment 10645
[I] 6492 | [TRANSPORT] ACK received for segment 10645
[6492] Data sent with ACK reception
\end{lstlisting}
}

Una clara limitació ha estat la manca de dispositius. A causa de només disposar d'un \est{gateway} i de dos dispositius, no s'ha pogut verificar el funcionament del protocol en grans xarxes, o en topologies complexes. 
...
\begin{itemize}
    \item Els temps d'espera de recepció d'\acro{ack} a la capa d'accés al medi no eren coherents, i resultaven en \est{timeouts} prematurs. Això es devia a iniciar el temporitzador de recepció d'\acro{ack} quan es començava la transmissió, i no en la seva finalització.
    \item Després de realitzar una verificació de l'estat del canal (a través de \acro{cad}), el transductor no es posava en estat de recepció. Això provocava que no es produís la interrupció de recepció i, per tant, que no es processessin els missatges rebuts.
    \item Configuració de la ràdio en LoRaWAN i LoRa. Després de realitzar una transmissió a través de LoRaWAN, no es rebien missatges enviats a través de LoRa. Això es devia al fet que la ràdio quedava amb la configuració de LoRaWAN, i no es tornava a configurar per a LoRa. Es va solucionar a través de la capa d'abstracció \est{LoRa}, superior a \est{LoRaWAN} i {LoRaRAW}.
\end{itemize}

Després de les verificacions, cada capa del protocol satisfà els requisits prèviament definits, amb la fiabilitat i el baix consum de les transmissions com a objectius principals. Tot i que no s'han pogut realitzar proves en entorns reals i topologies més complexes, es considera que el protocol estableix una base funcional i estable per a la seva aplicació.
...
Una segona opció consisteix a dividir les transmissions de dades en dos blocs: en el primer bloc transmeten els dispositius en posició senar (\fitx{D1, D3}, etc.), i en el segon bloc els dispositius en posició parell (\fitx{D2, D4}, etc.). Per si sol això no és suficient, ja que dos veïns continuen transmetent simultàniament quan un envia el missatge de sincronització i l'altre les dades. Per evitar-ho, es pot aplicar un marge abans de transmetre les dades, de com a mínim el temps de transmissió del missatge de sincronització. 
...
\begin{enumerate}
    ...
    \item La durada del cicle de funcionament normal ---des que surt del mode de baix consum fins que torna a iniciar-lo--- és molt elevat. El node més proper al \est{gateway} ha d'esperar-se que el node més allunyat realitzi la seva transmissió, i aquesta s'encamini a través de tota la xarxa.  
...
\begin{enumerate}
    ...
    Establir un altre dispositiu per enviar primer les dades és incoherent, ja que implicaria que hauria de transmetre les seves dades en sentit oposat al \est{gateway}, les quals després haurien de ser encaminades en sentit contrari.
...
\end{tabular}
    ...
    \caption{Transmissió de dades acumulades. Dispositiu de referència intermedi.}
...
\begin{itemize}
    ...
    \item La resta de dispositius, en sortir del mode de baix consum, s'esperen a rebre el missatge de dades, que indica també l'instant de sincronització. Llavors hi afegeixen les seves dades, el transmeten, i comencen el mode de baix consum.
\end{itemize}
...
\\
D'ara endavant es considerarà l'instant en què es rep el missatge de sincronització com a $t_s$, i l'instant previ a iniciar el mode de baix consum com a $t_d$. El temps de funcionament normal, $t_n$, és la diferència entre ambdós: $t_n = t_d - t_s$. És important mencionar que un dispositiu coneix, en tot moment, el temps que ha transcorregut des que s'ha inicialitzat i, per tant, pot determinar l'instant de temps de cada un dels anteriors esdeveniments.
...
La capa d'accés al medi d'un dispositiu pot necessitar realitzar retransmissions si no rep el missatge de reconeixement, o aplicar \acro{beb} si el canal està ocupat per un altre dispositiu ---aliè a la xarxa---. Aquestes retransmissions impliquen que el missatge de dades arribi amb retard al següent dispositiu i, en conseqüència, que aquest també el propagui més tard, afectant la resta de la xarxa. 
...
Per establir aquest temps d'espera cal considerar de nou el temps de tolerància ($t_t$) exposat anteriorment, que expressa la diferència entre el primer i últim missatge de dades vàlid que un dispositiu pot rebre. Per un dispositiu, suposem el següent:
\begin{enumerate}
...
En el cas que es produís una fi de temps d'espera, el dispositiu assumeix que la seva sincronització era vàlida i que l'error introduït és petit. Per aquest motiu, el càlcul de temps de baix consum el realitzarà com si fos el dispositiu de referència. 
\newline
\newline
Després de considerar aquestes no-idealitats, és necessari modificar l'expressió de càlcul dels temps de baix consum:
\begin{itemize}
    \item \est{Referència}. En tractar-se del dispositiu de referència, i com que és qui marca la referència, el seu temps de cicle no es veu modificat. Així, continua sent determinat per
    \begin{equation}
        t_b = t_c - t_d
...
\begin{itemize}
    \item \emph{Dades}. Les dades que s'envien en el missatge. En aquest camp cada dispositiu hi inclou les seves dades abans de transmetre-les. Té una longitud màxima de \SI{238}{\byte}, tenint en compte que la capa de transport permet un màxim de \SI{240}{\byte}, i que s'han de reservar \SI{2}{\byte} per als camps de longitud i ordre. 
És important mencionar que tots els dispositius de la xarxa coneixen la quantitat de dades que han d'enviar, i els dispositius no estan restringits a haver de transmetre tots la mateixa quantitat de dades. Així, un dispositiu podria estar configurat per haver de transmetre \SI{2}{\byte}, mentre que un altre \SI{10}{\byte}. Tot i això, aquesta situació requeriria més complexitat en el servidor d'aplicació LoRaWAN encarregat de descodificar les dades, que hauria de considerar aquestes longituds diferents. 
...
\begin{itemize}
    ...
    \item \emph{Propagar}. En aquest estat el dispositiu propaga el missatge de dades cap al següent dispositiu. Executa el \est{callback} d'obtenció de dades, i utilitza la interfície de la capa de transport per enviar el missatge. Quan aquesta mateixa capa notifica que la transmissió ha finalitzat (sigui amb èxit o error), canvia al següent estat.
...
Amb tots els dispositius correctament configurats i programats, es pot iniciar el sistema. Els dispositius s'han d'iniciar de forma inversa a com es propaguen les dades, és a dir, començant pel dispositiu més llunyà al de referència (i més proper al \est{gateway}), i acabant pel de referència. Això és necessari per garantir que cada dispositiu trobi el seu veí actiu quan s'inicia, i redueix el temps que els dispositius es troben en l'estat d'espera de sincronització inicial. Si es posés en marxa en l'ordre contrari, el dispositiu de referència començaria a transmetre quan la resta encara estarien apagats. En encendre'ls posteriorment, aquests quedarien esperant una sincronització que ja s'hauria produït, i haurien de romandre actius gairebé tot un cicle complet abans de rebre'l.
...
\begin{itemize}
    \item \emph{Referència}. Si es vol afegir un dispositiu nou a l'extrem més allunyat al \est{gateway}, aquest actuarà com a nou iniciador de la sincronització. 
    \begin{enumerate}
...
Si la quantitat de dispositius anteriors és major que la que s'ha configurat, el càlcul del temps de tolerància serà incorrecte, i no es pot garantir la sincronització. Si això passés, seria necessari modificar aquest paràmetre en tots els dispositius següents al que s'afegeix, i reiniciar-los tots. Pel que fa als dispositius anteriors, aquest valor seria el mateix i, per tant, no és necessari modificar-lo. Per aquest motiu, si es preveu afegir nous dispositius, es recomana establir aquest paràmetre amb un valor superior al necessari.

Això es pot evitar si els dispositius s'afegeixen en l'extrem més proper al \est{gateway}, ja que aquest seria l'últim dispositiu de la cadena. En aquest cas, seria únicament necessari modificar el dispositiu anterior al que s'afegeix per indicar-li el nou veí, i establir que no té capacitats LoRaWAN.

\subsubsection{Resincronització de la xarxa}
Tot i que l'aplicació intenta garantir la sincronització de tots els dispositius de la xarxa, i la robustesa i fiabilitat, és possible que es produeixin errors que puguin dur a una dessincronització total de la xarxa. En tal cas, caldria reiniciar tots els dispositius de la xarxa, i iniciar-los de nou com si fos la primera vegada. En aquest procediment també es recomana detectar l'error que ha provocat la dessincronització, i corregir-lo si és possible. A continuació es mostren algunes possibles causes d'error, i la seva solució:
\begin{itemize}
    \item Mal enllaç entre dispositius. Si la dessincronització és causada per un mal enllaç entre dispositius, es podrien modificar els paràmetres de transmissió LoRa, o bé afegir un dispositiu intermedi per millorar la comunicació.
...
Les proves s'han realitzat utilitzant dos microcontroladors \acro{esp32}, cadascun equipat amb un transductor LoRa \acro{sx1262}. Per verificar el correcte funcionament a través de LoRaWAN, es disposava de connectivitat a un \est{gateway} Mikrotik wAP-LR8. Es poden observar els dispositius usats a la \autoref{fig:dispositius_proves}.
...
Abans d'iniciar les proves, es va determinar quin era l'error del rellotge dels microcontroladors en mode de baix consum. Es va desenvolupar un petit programa que posava el dispositiu en mode de baix consum durant un temps conegut, enviant un missatge de depuració abans i després d'aquest mode. Gràcies a les marques de temps, era possible determinar la durada real del mode de baix consum, i calcular l'error en \acro{ppm} a partir de la durada esperada. Aquest experiment es va repetir per a diferents temps de cicle, i múltiples vegades. Es pot observar el codi i els resultats obtinguts a la carpeta \fitx{tests/CalculErrorPPM}.
...
S'observa com els resultats d'error obtingut no es corresponen amb cap dels errors obtinguts prèviament, indicant que per algun motiu aquest error ha augmentat en implementar el protocol i l'aplicació de baix consum. A més, sorprèn el fet que el segon dispositiu tingui un error més, tenint en compte que la implementació entre dispositius acaba sent molt semblant. Aquests errors no són negligibles, tenint en compte que l'error s'ha més que duplicat respecte dels obtinguts prèviament i, a més, semblen ser imprevisibles.

Per descobrir l'origen d'aquest error, es va dur a terme una bateria de proves (\fitx{tests/provesOrigenErrorClock.cpp}). Les proves consistien a simular el funcionament de la capa d'aplicació per a cada dispositiu, però sense utilitzar-la directament. Les proves realitzades van ser:
\begin{itemize}
...
Per verificar aquesta hipòtesi, es va realitzar una nova prova, modificant el rendiment de cicle i obtenint l'error en cada un d'ells. És a dir, per a un temps de cicle determinar (2.5 minuts), es va modificar el temps que el microcontrolador estava actiu, i trobant-se la resta del cicle en mode de baix consum. Els resultats obtinguts, per a cada microcontrolador, es poden observar a la \autoref{tab:app_resultatsRendiment}, els quals verifiquen la hipòtesi plantejada: a major rendiment de cicle, major és el temps que el microcontrolador es troba actiu, i major és l'error del rellotge. És important tenir en compte que, tot i que l'error en segons disminueix, l'error en \acro{ppm} depèn del temps en baix consum, el qual també disminueix, i no del temps de cicle total.
...
\centering
    \begin{tabular}{rrSSS}
        ...
        \midrule
        \multirow{2}{*}{1}  & \multirow{2}{*}{0.67}  & \footnotesize 148.990 & \footnotesize 1.643201 & \footnotesize 11029 \\
                            &                        & \footnotesize 148.962 & \footnotesize 1.618333 & \footnotesize 10864 \\
        \multirow{2}{*}{2}  & \multirow{2}{*}{1.33}  & \footnotesize 147.980 & \footnotesize 1.749711 & \footnotesize 11824 \\
                            &                        & \footnotesize 147.962 & \footnotesize 1.851677 & \footnotesize 12515 \\
        \multirow{2}{*}{10} & \multirow{2}{*}{6.67}  & \footnotesize 139.980 & \footnotesize 2.095421 & \footnotesize 14969 \\
                            &                        & \footnotesize 139.961 & \footnotesize 2.051842 & \footnotesize 14660 \\
        \multirow{2}{*}{25} & \multirow{2}{*}{16.67} & \footnotesize 124.980 & \footnotesize 2.066104 & \footnotesize 16531 \\
                            &                        & \footnotesize 124.960 & \footnotesize 2.032617 & \footnotesize 16266 \\
        \multirow{2}{*}{50} & \multirow{2}{*}{33.33} & \footnotesize 99.980  & \footnotesize 1.680000 & \footnotesize 16803 \\
                            &                        & \footnotesize 99.961  & \footnotesize 1.683075 & \footnotesize 16837 \\
        \multirow{2}{*}{75} & \multirow{2}{*}{50.00} & \footnotesize 74.980  & \footnotesize 1.303489 & \footnotesize 17384 \\
...
Per acabar, es va verificar que aquesta aplicació aconseguís reduir satisfactòriament el consum. Per fer-ho, es va comparar el consum d'un dispositiu sense implementar aquesta solució, que es trobava en mode actiu durant tot el cicle de 5 minuts, amb un altre que sí que implementava l'aplicació de baix consum ---utilitzant els resultats de la primera prova---. En la comparació no es va considerar el consum durant una transmissió, ja que en qualsevol de les situacions ambdós dispositius consumirien el mateix per realitzar-la.

Per tal de mesurar els consums, és necessari determinar el corrent d'alimentació. Abans de fer-ho, es va calcular la intensitat utilitzada pel LED vermell de la placa de desenvolupament, que indica que està alimentada i sempre es troba actiu, obtenint un valor de \SI{1.7}{\milli\ampere}.
Seguidament es va mesurar el corrent d'alimentació, obtenint que en mode actiu el dispositiu consumia aproximadament \SI{80}{\milli\ampere}, i en mode de baix consum aquest es reduïa a \SI{13}{\milli\ampere}. Sense considerar el consum del LED, el consum en mode actiu és de \SI{78.3}{\milli\ampere}, i en mode de baix consum és de \SI{11.3}{\milli\ampere}.

Tenint en compte que el temps de funcionament normal mitjà va ser de 23.3 segons, podem aproximar el consum per cicle de cada situació:
\begin{itemize}
    \item \emph{Sense aplicació}. El consum per cicle és directament $78.3 \cdot \frac{5}{60} = \num{6.53}$ mAh.
    \item \emph{Amb aplicació}. Cal considerar el consum en mode actiu, i en mode de baix consum. Així, és $78.3\cdot\frac{23.3}{60\cdot60}+11.3\cdot\frac{5\cdot60-23.3}{60\cdot60}=1.38$ mAh.
\end{itemize} 

Amb aquests resultats, es verifica com l'aplicació de baix consum ha reduït el consum en un $\frac{6.53-1.38}{6.53}\cdot100=78.9\%$.

Es poden aproximar altres resultats, considerant que el temps de funcionament normal és constant ---només vàlid si l'error del rellotge és reduit---. Per exemple, per un temps de cicle d'1 hora, la reducció del consum seria d'un 85\%, i per un cicle d'un dia del 85.5\%.  

És important tenir en compte que aquests resultats són únicament vàlids per aquesta situació, i que depenen no només del temps de cicle, sinó que també, per exemple, de la quantitat de dispositius o paràmetres del protocol, ja que fan modificar el temps de tolerància i, per tant, el temps de baix consum. 

Amb el maquinari adequat, amb el qual es podrien obtenir consums en mode de baix consum de menys d'\SI{1}{\milli\ampere}, s'obtindrien reduccions molt majors, de l'ordre del 98\% en un cicle d'1 hora.

% Dir que segons hardware, es poden aconseguir consums en baix consum de micros, fent aque la redeucció sigui encara majro

\chapter{Conclusions}
Aquest treball ha presentat el disseny i la implementació d'un protocol de comunicació basat en LoRa, amb capacitats d'encaminament estàtic i compatible amb LoRaWAN, aconseguint reduir la dependència de \est{gateways} LoRaWAN en els dispositius de monitoratge. Això s'ha aconseguit fent possible que un dispositiu sense connectivitat directa amb un \est{gateway} transmeti les dades a través d'un o diversos dispositius intermedis, que actuen com a nodes encaminadors fins a arribar al \est{gateway}. El protocol s'ha limitat a xarxes estàtiques, una característica pròpia de les aplicacions de monitoratge, on els dispositius tenen una ubicació fixa i es vol monitorar una situació concreta de forma continuada. Gràcies a això, s'elimina la necessitat d'un protocol de descobriment de rutes, simplificant-ne així la implementació i manteniment.

Les proves realitzades, que s'han vist limitades a la disponibilitat d'únicament dos dispositius, han permès validar el funcionament del protocol i demostrar-ne la seva viabilitat. Els mecanismes d'accés al canal compartit, així com els mecanismes de retransmissió, asseguren una comunicació fiable i robusta, mentre que l'encaminament permet augmentar el rang de comunicació dels dispositius, més enllà del seu abast directe. Gràcies als segments de la capa de transport, s'aconsegueix una comunicació fiable d'extrem a extrem, i la capa d'aplicació permet la compatibilitat i adaptació del protocol a diferents escenaris i necessitats. 

A partir del protocol, s'ha creat una aplicació encarregada de reduir el consum energètic dels dispositius. Aquesta s'ha limitat a un tipus concret de xarxes: les xarxes lineals, especialment útils en entorns ferroviaris o agrícoles. El funcionament de l'aplicació es basa en un mecanisme de sincronització, on un dispositiu inicia el cicle de funcionament i la resta es mantenen a l'espera de rebre un missatge de sincronització. Per minimitzar el nombre de transmissions, s'aprofita el missatge de sincronització per enviar les dades de cada dispositiu. Així, en rebre la sincronització, el dispositiu hi afegeix les seves dades i propaga el missatge al següent dispositiu de la xarxa.

En aquesta aplicació s'ha buscat la robustesa i fiabilitat a llarg termini, preferint una pèrdua puntual de dades abans que la fallada total de la xarxa. Un clar exemple és la implementació del temps d'espera màxim per rebre la sincronització, el qual evita que els dispositius es quedin en mode de recepció indefinidament, i prenguin la iniciativa d'iniciar ells la sincronització. Això permet identificar on s'ha produït una fallada, i evita perdre les dades de tots els dispositius de la xarxa, així com conservar energia. També s'han considerat altres situacions, incloent-hi la fragmentació de dades, o errors en la sincronització causats per reintents de transmissió a la capa d'accés al medi. Per evitar aquest últim, s'ha implementat el que s'ha denominat «temps de tolerància», que es correspon a l'interval entre recepcions de sincronitzacions vàlides.

Durant les proves de validació, s'ha detectat una limitació important en la precisió dels osci\l.ladors dels microcontroladors utilitzats en mode de baix consum. Els seus elevats errors ($\pm 5000$ ppm) i alta sensibilitat a la temperatura ($1000$ ppm/ºC) fan necessari deixar marges de temps de seguretat elevats, penalitzant l'eficiència energètica. Com a solució a aquest problema, es proposa l'ús d'osci\l.ladors de cristall, que presenten errors molt més reduïts i constants, cosa que permet sincronitzacions més precises.

Les proves realitzades han demostrat que l'aplicació proposada aconsegueix reduir el consum energètic dels dispositius. De forma experimental, i amb el maquinari utilitzat, s'ha observat una reducció del consum del 85\% en un cicle d'1 hora. Aquesta reducció podria ser encara més elevada amb els components més eficients ---amb consums molt reduïts en mode de baix consum---, sent possible assolir reduccions superiors al 98\%. 

En conjunt, aquest treball ha permès demostrar la viabilitat d'una solució de comunicació eficient per a entorns de monitoratge basats en LoRa, buscant l'equilibri entre simplicitat, fiabilitat i baix consum energètic. Si bé encara existeixen possibles millores futures i proves en entorns més extensos i situacions reals, el sistema desenvolupat ofereix una base sòlida sobre la qual construir aplicacions reals.

\chapter{Treball futur}
La solució proposada ha assolit els objectius principals del treball, desenvolupant un protocol funcional i un mecanisme per a la reducció del consum. A més, s'identifiquen diverses línies de treball futur que poden aportar-hi noves funcionalitats:
\begin{itemize}
    \item \emph{Altres tecnologies de comunicació}. El sistema actual es basa a utilitzar LoRaWAN per disposar de les dades de la xarxa. Una extensió possible seria substituir aquesta tecnologia per d'altres, com ara 4G, podent ser útil en entorns on LoRaWAN no és viable. Gràcies a la modularitat del protocol, aquesta implementació seria relativament senzilla, i podria aprofitar les mateixes capes d'encaminament i transport.
    ...
    \item \emph{Control extern de l'alimentació}. Una millora que podria reduir més el consum energètic és l'ús d'un circuit integrat capaç de desconnectar completament l'alimentació del microcontrolador i de la ràdio. Aquest circuit permetria definir un temps de desconnexió, al final del qual s'activaria de nou. El microcontrolador podria indicar quan desactivar l'alimentació, just abans del mode de baix consum. Aquesta estratègia permetria aconseguir un estalvi energètic molt elevat precisament durant el mode de baix consum, que és quan l'aplicació aconsegueix reduir el consum energètic, fent-ho especialment útil per dispositius que requereixen una gran autonomia.  
\end{itemize}
