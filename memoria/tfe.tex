...
\begin{resum}
% Aquest treball presenta el disseny i implementació d'un protocol de comunicació LoRa amb encaminament estàtic, destinat a xarxes de dispositiusde monitoratge. L'objectiu és reduir la dependència dels \est{gateways} en entorns on la presència humana és limitada, com ara túnels o vies de tren. El protocol permet que els nodes transmetin dades de manera eficient, aprofitant camins de comunicació predefinits.
\end{resum}

\begin{abstract}
% This work presents the design and implementation of a LoRa communication protocol with static routing, aimed at monitoring device networks. The goal is to reduce the dependency on gateways in environments where human presence is limited, such as tunnels or railway tracks. The protocol allows nodes to transmit data efficiently, leveraging predefined communication paths.
\end{abstract}
