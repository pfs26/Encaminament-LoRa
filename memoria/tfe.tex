...
\begin{table}
    ...
    \end{tabular}
    \caption{Matriu d'encaminament dels dispositius d'exemple. La ce\l.la $(x, y)$ indica el dispositiu que $x$ ha d'utilitzar per enviar un paquet a $y$.}
    \label{tab:routing_taula_enc_exemples}
\end{table}
...
Un segon exemple de transmissió es realitza quan \fitx{0x02} vol enviar un paquet al \est{gateway}. 

El dispositiu 2 consulta la taula d'encaminament i veu que el següent salt és el dispositiu 3. Aquest rep el paquet i consulta la seva taula, veient que el següent salt és el \est{gateway}. El dispositiu 3 transmet el paquet al \est{gateway}, que a la vegada l'envia al dispositiu 4.
...
\begin{figure}[h]
    ...
    \draw ($(dev2.south) + (0,0)$) -- ($(dev2.south) + (0,-5)$);
    \draw ($(dev3.south) + (0,0)$) -- ($(dev3.south) + (0,-5)$);
    \draw ($(gw.south) + (0,0)$) -- ($(gw.south) + (0,-5)$);
...
\begin{itemize}
    ...
    % \item \emph{Longitud}. Longitud de les dades. \textcolor{red}{TODO: AIXÒ NO ÉS NECESSARI, VEURE ISSUE A GITHUB}
