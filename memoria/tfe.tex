...
\begin{resum}
Aquest treball presenta el disseny i implementació d'un protocol de comunicació basat en LoRa amb encaminament estàtic, destinat a xarxes de dispositius de monitoratge en entorns extensos o aïllats, on la presència humana és limitada. L'objectiu d'aquest és reduir la dependència dels \est{gateways} LoRaWAN, aprofitant que els dispositius són sovintment estàtics i permeten l'ús de rutes de comunicació prèviament definides.

Sobre aquest protocol s'ha desenvolupat una aplicació optimitzada per a xarxes lineals, una topologia útil en vies de tren o túnels, centrada en minimitzar el consum energètic dels dispositius. Aquesta solució es basa en una estratègia de sincronització entre nodes, que permet coordinar les seves transmissions i reduir el temps actiu dels dispositius, sense renunciar a la fiabilitat o als mecanismes d'encaminament que el protocol ofereix. Les proves realitzades han demostrat que la proposta és viable, escalable i eficient per a entorns on el consum energètic i la cobertura són crítics.
...
Les proves s'han realitzat utilitzant dos microcontroladors \acro{esp32}, cadascun equipat amb un transductor LoRa \acro{sx1262}. Per verificar el correcte funcionament a través de LoRaWAN, es disposava de connectivitat a un \est{gateway} Mikrotik wAP-LR8.
...
Per descobrir l'origen d'aquest error, es va dur a terme una bateria de proves (\fitx{tests/provesOrigenErrorClock.cpp}). Les proves consistien en simular el funcionament de la capa d'aplicació per a cada dispositiu, però sense utilitzar la capa d'aplicació. Les proves realitzades van ser:
\begin{itemize}
    ...
    \item \emph{Transmissió després de retard}. Simula el comportament del dispositiu que espera la sincronització, el qual espera rebre les dades després de sortir del mode de baix consum, per després propagar-les i iniciar de nou el mode de baix consum. Els resultats es corresponien amb els obtinguts en executar l'aplicació, amb errors de 16000 \acro{ppm}. Es va descartar que l'error en aquest dispositiu s'originés a la capa d'aplicació, per exemple per un mal càlcul del temps de baix consum.
...
Per verificar aquesta hipòtesi, es va realitzar una nova prova, modificant el rendiment de cicle i obtenint l'error en cada un d'ells. És a dir, per a un temps de cicle determinar (2.5 minuts), es va modificar el temps que el microcontrolador estava actiu, i trobant-se la resta del cicle en mode de baix consum. De nou, per a cada rendiment de cicle es va determinar l'error del rellotge. Els resultats obtinguts, per a cada microcontrolador, es poden observar a la \autoref{tab:app_resultatsRendiment}, els quals verifiquen la hipòtesi plantejada: a major rendiment de cicle, major és el temps que el microcontrolador es troba actiu, i major és l'error del rellotge. És important tenir en compte que tot i que l'error en segons disminueix, l'error en \acro{ppm} depèn del temps en baix consum, el qual també disminueix, i no del temps de cicle total.
