...
\begin{itemize}
    \item $N$: nombre de dispositius anteriors al dispositiu pel qual es calcula el temps de tolerància.
...
Tot i existir mecanismes que intenten assegurar la integritat dels missatges, és possible que un dispositiu no rebi correctament les dades. Això pot ser degut a interferències, o bé a que el dispositiu que ha d'enviar les dades s'ha espatllat

Aquest cas provocaria que un dispositiu que no és el de referència quedés esperant la recepció de dades indefinidament, sense entrar en mode de baix consum. A més, implicaria la pèrdua de les dades de tota la xarxa. Per evitar-ho, s'ha implementat un temporitzador que, en cas de no rebre les dades en aquest temps, el dispositiu assumeix que no les rebrà, i pren la iniciativa d'iniciar ell la transmissió de dades cap al següent dispositiu.

Els següents dispositius rebrien el nou missatge de dades, i actuarien de forma normal. Com que els dispositius més propers al de referència inicien el mode de funcionament normal abans, també finalitzaran aquest temps d'espera abans, garantint que un dispositiu més llunyà no prengui la iniciativa d'iniciar la transmissió de dades abans que un de més proper.
...
\begin{itemize}
    ...
    \item Permet detectar en quin punt de la xarxa hi ha hagut l'últim error. Si al servidor d'aplicació de LoRaWAN rebem un missatge amb \SI{10}{\byte} de dades, i sabem que cada dispositiu afegeix \SI{2}{\byte}, es dedueix que els últims 5 dispositius funcionen adequadament, i que el problema es troba en el sisè dispositiu, o en l'enllaç entre aquest i el cinquè. Tot i això, no es pot assegurar que aquest sigui l'únic error, ja que en podrien existir més en dispositius anteriors al sisè.  
\end{itemize}
...
Així, s'observa com el límit de temps en què pot rebre el missatge de dades és $t_2 + t_t$. Per tant, el temps d'espera és el mateix que el temps de tolerància, centrat en l'instant on hauria de finalitzar el cicle ($t_2$). Cal tenir en compte, però, que surt del mode de baix consum a $t_2 - t_t$ i, per tant, el temps d'espera programat en el temporitzador en iniciar el funcionament normal ha de ser de $2 \cdot t_t$.

En el cas que es produís una fi de temps d'espera, el dispositiu assumeix que la seva sincronització era vàlida i que l'error introduit és petit. Així, el càlcul de temps de baix consum el realitzarà com si fos el dispositiu de referència. 
\\ \\
...
Per tal de facilitar l'obtenció de les dades, s'ha implementat un \textit{callback} que pot ser configurat per qualsevol altra part del programa. Aquest \textit{callback} rep com a paràmetre un punter a les dades del missatge i és l'encarregat d'afegir-hi la informació que es vol transmetre. De manera automàtica, la capa d'aplicació executa aquest \textit{callback} just abans de propagar les dades.

Aquesta estratègia ofereix flexibilitat, ja que permet que qualsevol part del programar defineix-hi com s'obtenen les dades a transmetre, sense necessitat de modificar la capa d'aplicació. A més, també treu responsabilitat a la capa que obté les dades, que no s'ha de preocupar de quan s'han d'enviar: la capa d'aplicació és qui li ho notificarà.
...
\begin{itemize}
    ...
    \item \emph{Dormir}. En aquest estat el dispositiu entra en mode de baix consum, determinant la seva durada a través de les expressions \ref{eq:calculSleepRef} i \ref{eq:calculSleepList}.
\end{itemize}
...
\begin{figure}
    ...
    \begin{tikzpicture}[shorten >=1pt, node distance=4cm, on grid, auto]
        ...
        (prop) edge[bend left=15] node[sloped, above] {\tiny tx ok} (sleep)

        (prop) edge[bend right=15] node[sloped, above] {\tiny tx err} (sleep)
        (sleep) edge node[above] {\tiny fi timer} node {\tiny \emph{inici timer sync}} (sync)
        (sync) edge[bend left=15] node[sloped, above] {\tiny sync} (prop)
        (sync) edge[bend right=15] node[sloped, above] {\tiny fi timer || iniciador}
...
% Afegir nous dispositius
    % explicar quins dispositius s'han de modificar
